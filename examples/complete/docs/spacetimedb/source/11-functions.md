...
| ------------------------------ | -------- | ---------- | ----- |
| Read from tables               | Yes      | Yes        | Yes   |
| Write to tables                | Yes      | Yes        | No    |
| Runs in transaction by default | Yes      | No         | Yes   |
| Atomic                         | Yes      | (manual)   | Yes   |
| Deterministic                  | Yes      | No         | Yes   |
| External I/O (HTTP, etc.)      | No       | Yes        | No    |
| Side-effecting                 | No       | Yes        | No    |
...
Views will automatically update clients when underlying data changes just like tables, making them ideal for real-time computed data.
