...
Unlike reducers, [procedures](/docs/functions/procedures) don't automatically run in transactions. Procedures can run transactions, but must manually open them using `with_tx`. This gives procedures more flexibility:
...
- Each `with_tx` call creates a new transaction that commits independently
...
- Use `Result<(), String>` in Rust to return clear error messages
