...
A database's schema and business logic is specified by a piece of software called a **module**. Modules can be written in Rust.
...
A SpacetimeDB **table** is a SQL database table. Tables are declared in a module's native language. For instance, in Rust, a table is declared like so:
...
Connected [clients](/docs/clients) can call reducers to interact with the database.
...
Clients are written using the [client-side SDKs](/docs/clients). The `spacetime` CLI tool allows automatically generating code that works with the client-side SDKs to talk to a particular database.
