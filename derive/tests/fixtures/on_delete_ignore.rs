//! Covers `on_delete = Ignore`: deleting a referenced row leaves the referencing rows
//! untouched, so the generated strategy implementation is intentionally empty.
//!
...
pub struct Book {
    ...
    #[foreign_key(path = self, table = author, column = id, on_delete = Ignore)]
    pub author_id: u64,
}
