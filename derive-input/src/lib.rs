...
mod tests {
    ...
    fn test_last_dsl_attribute_detection() {
        ...
        assert_eq!(
            dsl_attributes.len(),
            2,
            "Should find exactly 2 DSL attributes"
        );
        ...
        assert!(
            !is_first_last,
            "First DSL attribute should not be detected as last"
        );
        ...
        assert!(
            is_second_last,
            "Second DSL attribute should be detected as last"
        );
    }
    ...
    fn test_single_dsl_attribute_is_last() {
        ...
        assert_eq!(
            dsl_attributes.len(),
            1,
            "Should find exactly 1 DSL attribute"
        );
