...
pub(in crate::internal) fn for_single_column_index(
    ...
    let method_name = format!("get_{}_by_{}", &spacetimedsl_table.plural_name, column_name).into();

    let method_arg;

    let table_type: Type = parse_str(&rust_struct.name).expect("get_many.for_single_column_index");
    let return_type = get_return_table_type_iterator(&table_type);
...
pub(in crate::internal) fn for_multi_column_index(
    ...
    let index_columns = match &multi_column_index.index_type {
        ...
        i => {
            panic!(
                "There shouldn't be an index with another type when get_many.for_multi_column_index is running. Found: {:#?}",
                i
            )
        }
    };
    ...
    let table_type: Type = parse_str(&rust_struct.name).expect("get_many.for_multi_column_index");
    let return_type = get_return_table_type_iterator(&table_type);
    ...
    for column in columns {
        ...
        let column_type: Type = parse_str(&column.rust_field.type_name_or_path)
            .expect("get_many.for_multi_column_index");
