...
use crate::{
    ...
    internal::dsl::quote::{
        ...
        get_method_arg_vec, get_return_table_type_option_iterator,
    },
};
...
pub(in crate::internal) fn for_single_column_index(
    ...
    let doc_comment = format!(
        "Get all Option<{}> rows inside the {} table filtered by {} values.",
    ...
    let method_arg_name = format!("{}_vec", column_name);
    let mut method_arg = quote! { #method_arg_name: };

    let return_type = get_return_table_type_option_iterator(&rust_struct.name);
    ...
    match &spacetimedsl_column.wrapper_type {
        Some(wrapper_type) => {
            ...
            if spacetimedsl_column.is_option {
                method_arg.append_all(get_method_arg_vec(get_method_arg_into_wrapper_type_option(
                ...
                method_arg.append_all(get_method_arg_vec(get_method_arg_into_wrapper_type(
                    wrapper_type_name_or_path,
                )));
        ...
        None => {
            method_arg.append_all(get_method_arg_vec(get_method_arg_column_type(
                &rust_field.type_name_or_path,
            )));
    ...
    let method_impl = quote! {
        #into_option
        let mut result: Vec<Option<#table_name>> = vec![];

        for #column_name in #method_arg_name {
