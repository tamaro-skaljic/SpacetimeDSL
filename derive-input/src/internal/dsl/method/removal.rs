...
pub(in crate::internal) fn for_removal_many(
    ...
    let reported_strategy = match removal {
        ...
        Removal::Soft => todo!("Task 8 reports SoftDelete here"),
    };
    ...
    // Variation point: which pair of hooks runs.
    let (declared_before_hook, declared_after_hook) = match removal {
        Removal::Hard => (
            &spacetimedsl_table.hooks.before_delete,
            &spacetimedsl_table.hooks.after_delete,
        ),
        Removal::Soft => todo!("Task 8 runs the soft delete hooks here"),
    };

    let before_delete_hook = hook_tokens(declared_before_hook, |hook_function_name| {
        let hook_call =
            runtime::dsl_method_hooks_call(hook_function_name, &quote! { self, &row_to_delete });
    ...
    let after_delete_hook = hook_tokens(declared_after_hook, |hook_function_name| {
        let hook_call =
            runtime::dsl_method_hooks_call(hook_function_name, &quote! { self, &row_to_delete });
    ...
    let delete_many_impl = match removal {
        ...
        Removal::Soft => todo!("Task 8 writes the marker of every matched row here"),
    };
    ...
    let (doc_comment, method_name) = match removal {
        ...
        Removal::Soft => todo!("Task 8 names the soft delete method here"),
    };
...
pub(in crate::internal) fn for_removal_one(
    ...
    let delete_one_impl = match removal {
        ...
        Removal::Soft => todo!("Task 8 writes the marker of the matched row here"),
    };
